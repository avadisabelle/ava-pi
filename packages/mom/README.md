...
```bash
npm install @avadisabelle/ava-pi-mom
```
...
- run interactive coding agent session: `npx @avadisabelle/ava-pi-coding-agent`
- enter `/login` command
...
Update mom anytime with `npm install -g @avadisabelle/ava-pi-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
